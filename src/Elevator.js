...
function Elevator() {
    ...
    React.useEffect(() => {
        if (pendingRequests.length > 0) {
            const new_floor = pendingRequests[0];
            const availableLift = lifts.find((lift) => !lift.moving && !lift.arrived);
            if (availableLift) {
                callElevator(new_floor, availableLift);
                setPendingRequests((prev) => prev.slice(1));
            }
        }
    }, [pendingRequests, lifts]);

    const callElevator = (floor) => {

        const availableLifts = lifts.filter((lift) => !lift.moving && !lift.arrived);
        ...
        const liftIndex = lifts.indexOf(closestLift);
        const updatedLifts = [...lifts];
        const selectedLift = updatedLifts[liftIndex];

        const travelTime = minDistance * 500;
        selectedLift.time = travelTime / 1000;
        selectedLift.moving = true;
        selectedLift.destFloor = floor;
        console.log(updatedLifts);
        setLifts(updatedLifts);
        setTimeout(() => {
            selectedLift.arrived = true;
            selectedLift.moving = false;
            selectedLift.currentFloor = floor;
            selectedLift.time = null; 
            setLifts([...updatedLifts]);
            setTimeout(() => {
                selectedLift.arrived = false;
                setLifts([...updatedLifts]);
            }, 2000)
            lifts[liftIndex] = selectedLift;
        }, travelTime);
        selectedLift.floorarrived = true;
    };
